  ...
  useEffect(() => {
    const fetchTargetData = async () => {
      ...
      if (
        ...
        (otherUserData.friends && !otherUserData.friends[user.uid])
      ) {
        if (
          otherUserData.privacySettings &&
          otherUserData.privacySettings.directMessages === "block"
        ) {
          setChatPrivacyIsBlock(true);
        }
        if (
          otherUserData.privacySettings &&
          otherUserData.privacySettings.calls === "block"
        ) {
  ...
  const handleSend = async () => {
    if (blockExists) {
      ...
      toast({
        title: "Cannot send message",
        description:
          "The other user has set their privacy settings to block messages from non-friends.",
        variant: "destructive",
      });
      setText("");
      setFile({
  ...
  const handleStartCall = async () => {
    if (callPrivacyIsBlock) {
      toast({
        ...
        description:
          "The other user has set their privacy settings to block calls from non-friends.",
        variant: "destructive",
      });
      return;
    }
          ...
          <div className="grow h-screen ml-60 bg-dc-700 flex flex-col pt-6">
            ...
            <div className="w-full min-h-12 shadow-md flex items-center text-sm text-left px-4">
              <div className="bg-dc-900 rounded-full w-8 h-8 overflow-hidden">
                <img
                  src={targetUserData.imageUrl}
                  alt={targetUserData.uid}
                  className="w-full h-full object-cover"
                />
              </div>
              <p className="grow pl-4 font-semibold ">
            ...
            {ongoingCallExists ? (
              <>
                <div className="w-full h-full">
                  <MediaRoom chatId={chatId} audio={true} video={false} />
                </div>
              </>
            ) : (
              <>
